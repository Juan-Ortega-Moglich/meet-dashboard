...
import { Plus, Calendar, Clock, Video, X, Link2, ChevronDown, Bot, Loader2, CheckCircle2, AlertCircle } from "lucide-react";
...
// --- Mock Data (se reemplazará con Google Calendar) ---
...
const hosts: Host[] = [
  {
    ...
    upcomingMeetings: [
      { id: "w4", title: "Demo cliente Acme Corp", time: "2026-02-26 10:00", platform: "Google Meet", botStatus: "pendiente" },
      { id: "w5", title: "Planeación sprint 14", time: "2026-02-27 09:00", platform: "Zoom", botStatus: "pendiente" },
    ],
  },
  {
    id: "biofleming",
    ...
    upcomingMeetings: [
      { id: "b3", title: "Presentación trimestral", time: "2026-02-26 14:00", platform: "Google Meet", botStatus: "pendiente" },
    ],
  },
  {
    id: "inbest",
    ...
    meetingsToday: [
      ...
      { id: "i3", title: "Cierre de día", time: "17:00", platform: "Zoom", botStatus: "pendiente" },
    ],
    upcomingMeetings: [
      { id: "i4", title: "Capacitación CRM", time: "2026-02-26 11:00", platform: "Zoom", botStatus: "pendiente" },
      { id: "i5", title: "Review semanal", time: "2026-02-27 10:00", platform: "Google Meet", botStatus: "pendiente" },
      { id: "i6", title: "Sync con marketing", time: "2026-02-28 09:30", platform: "Google Meet", botStatus: "pendiente" },
    ],
  },
  {
    id: "operaciones",
    name: "Operaciones",
    meetingsToday: [
      { id: "o1", title: "Daily standup", time: "09:00", platform: "Google Meet", botStatus: "finalizado" },
      { id: "o2", title: "Revisión KPIs operativos", time: "11:00", platform: "Google Meet", botStatus: "finalizado" },
      { id: "o3", title: "Sync con logística", time: "16:00", platform: "Zoom", botStatus: "pendiente" },
    ],
    upcomingMeetings: [
      { id: "o4", title: "Planeación mensual", time: "2026-03-01 10:00", platform: "Google Meet", botStatus: "pendiente" },
    ],
  },
  {
    id: "andres",
    ...
    upcomingMeetings: [
      { id: "a2", title: "Entrevista candidato Sr.", time: "2026-02-26 15:00", platform: "Google Meet", botStatus: "pendiente" },
      { id: "a3", title: "Revisión presupuesto", time: "2026-02-27 11:00", platform: "Zoom", botStatus: "pendiente" },
    ],
  },
  {
    id: "pablo",
    ...
    meetingsToday: [
      ...
      { id: "p3", title: "Retro sprint 13", time: "16:30", platform: "Zoom", botStatus: "pendiente" },
    ],
    upcomingMeetings: [
      { id: "p4", title: "Workshop UX", time: "2026-02-26 09:00", platform: "Google Meet", botStatus: "pendiente" },
    ],
  },
];
...
export default function BotGrabacionPage() {
  ...
  useEffect(() => {
    fetchActiveBots();
    const interval = setInterval(fetchActiveBots, 10000); // poll every 10s
    return () => clearInterval(interval);
  }, [fetchActiveBots]);

  // Re-fetch bots when modal closes (bot may have been created)
  const handleModalClose = () => {
    setModalOpen(false);
    setTimeout(fetchActiveBots, 1000);
  };

  // Filter active bots (not done/fatal)
  const liveActiveBots = activeBots.filter(
    (b) => !["done", "fatal", "call_ended"].includes(b.status)
  );
  const completedBots = activeBots.filter(
    (b) => ["done", "call_ended"].includes(b.status)
  );
  ...
  return (
    <div>
      ...
      {/* Active Bots (real, from Recall.ai) */}
        ...
        <div className="mb-6 md:mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="ml-1 px-2 py-0.5 rounded-full bg-green-100 text-green-700 text-xs font-semibold">
              {liveActiveBots.length}
            </span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {liveActiveBots.map((bot) => (
      ...
      {completedBots.length > 0 && (
        <div className="mb-6 md:mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="ml-1 px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 text-xs font-semibold">
              {completedBots.length}
            </span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      ...
      {/* Reuniones de Hoy (mock — se reemplazará con Google Calendar) */}
      <div className="mb-6 md:mb-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="ml-1 px-2 py-0.5 rounded-full bg-blue-100 text-[#2055e4] text-xs font-semibold">
            {selectedHost.meetingsToday.length}
          </span>
        </div>

        {selectedHost.meetingsToday.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
            <p className="text-sm text-gray-400">No hay reuniones para hoy</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {selectedHost.meetingsToday.map((meeting) => (
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="ml-1 px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 text-xs font-semibold">
            {selectedHost.upcomingMeetings.length}
          </span>
        </div>

        {selectedHost.upcomingMeetings.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
            <p className="text-sm text-gray-400">No hay reuniones próximas</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {selectedHost.upcomingMeetings.map((meeting) => (
              <MeetingCard key={meeting.id} meeting={meeting} />
            ))}
          </div>
        )}
